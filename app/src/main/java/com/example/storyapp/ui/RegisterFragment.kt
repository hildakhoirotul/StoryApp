...
class RegisterFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.btnRegister.setOnClickListener {
            if (binding.edtName.text?.length ?: 0 <= 0) {
                binding.edtName.error = getString(R.string.required_field)
                binding.edtName.requestFocus()
            } else if (binding.edtEmail.text?.length ?: 0 <= 0) {
            ...
            } else if (binding.edtName.error?.length ?: 0 > 0) {
                binding.edtName.requestFocus()
    ...
    private fun playAnimation() {
        ...
        val register = ObjectAnimator.ofFloat(binding.textView, View.ALPHA, 1f).setDuration(500)
